...
'     doc.out(MARC(SOURCE[i], 463, i, 200,0,$a), Side1)
    if(MARC(SOURCE[i], 463, 0, 210,0,$d) != year)
    {
     year = MARC(SOURCE[i], 463, 0, 210,0,$d)
     doc.out(year, Side1)
     doc.out(MARC(SOURCE[i], 463, 0, 200,0,($a?". - "$a)), Side1)
    }
    doc.out(MARC(SOURCE[i], 463, 0, 200,0,($v?". - "$v)), Side1)
    if(i != qwerty - 1)
     doc.out("; ", Side1)
'   }
  }
...
k = SOURCE[i].count(686)
 ...
 {
  if(SOURCE[i].value(686, l, $2) == "rubbk") 
   dlina_stroki=dlina_stroki+sizeof(SOURCE[i].value(686,l,$a))
 }
 '--------------выводим пробелы
...
'-------------цикл по количеству 686 полей -
 for(var l =0; l< k; l =l +1)
 {
  if(SOURCE[i].value(686, l, $2) == "rubbk") 
    {	
	 var m = MARCCOUNT(SOURCE[i], 686,l,$a)
'-------------цикл по количеству $a подполей 
	 for(var j = 0; j < m; j= j+1)
	 {
	     doc.out(MARC(SOURCE[i], 686,j, $a[j]" "), Side1)
	    if (j != m-1)
	     doc.out("+ ", Side1)  
	 }
  if (l != k-1)
