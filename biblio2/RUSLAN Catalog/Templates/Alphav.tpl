...
define function main()
{
 ...
 doc.ZoneSeparate(Side1, 15, VER, Zone1, Zone2)
 doc.ZoneSeparate(Zone1, 75, HOR, Zone3, Zone4)
...
doc.setfont(Zone9,"Segoe UI", 10)
'для ББК
 doc.ZoneSeparate(Zone9, 95, VER, Zone9,Zone92) 
doc.setfont(zone92, "", -1, -1, 2)
'для ISBN
' doc.ZoneSeparate(Zone9, 90, VER, Zone9,Zone93) 

 doc.out(source.value(686, 0, $a), Zone3)
...
' конец вставки -предмет
 ...
 doc.out(source.value(093, 0,$a), Zone7)
 doc.out("\n\n\n\n\n\n\n\n", Zone7)
...
for(i =0; i < k; i = i + 1)
 {
  doc.out(source.value(999, i, $b), zone222)
 }

 doc.out(source.value(999, 0, "\t\t"$p), Zone3)
 

 'doc.out(source.value(100, 0, "\t\t"$a[0]), Zone3)
 'a=tostr(source.count(100, 0, $a))
 
 doc.out("\n", Sideleft)
 'doc.out( substring (source.value(100, 0, $a), symbol, 17, 2), Sideleft)
