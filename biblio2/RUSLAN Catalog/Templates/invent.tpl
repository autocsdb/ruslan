...
'если headerrequired не равен 1
else
{
	doc.clear()
	var i
	...
	for(i = 0; i < Sourcesize; i = i + 1)
	{
		...
		for(j = 0; j < k ; j = j + 1)
		{
			kolvo = kolvo + ToNumber(marc(source[i], 999, j, $v))
		}
		
		doc.out(tostr(kolvo), "f" + ToStr(i + baseCell))
		kolvo=0
		
		for(j = 0; j < 1 ; j = j + 1)                      'k заменили на 1
		{
			...
			add(record, marc(source[i], 999, j, $v))
			add(record, marc(source[i], 999, j, $u))
			add(record, marc(source[i], 210, 0, $d))
			add(recmas, record)
			add(nummas, marc(source[i], 999, j, $p))
			add(billmas, marc(source[i], 999, j, $e))
		}	  
...
'=
		for(i = 0; i < Sourcesize; i = i + 1)
		{
			...
			doc.out(number, "b" + ToStr(i + baseCell))
			doc.out(record[3], "c" + ToStr(i + baseCell))
			doc.out(record[4] + ", " + record[7], "d" + ToStr(i + baseCell))
			'год
			'summa = tonumber(record[5])*tonumber(record[6])
			'doc.out(tostr(summa), "f" + ToStr(i + baseCell))
		...
		doc.out("=sum(f3:f" + tostr(i + baseCell - 1) + ")", "f" + ToStr(i + baseCell))
		doc.drawborder("A" + ToStr(baseCell - 2) + ":h" + ToStr(i + baseCell - 1)) 
		if(i > 0)
		{
			doc.drawborder("B" + ToStr(baseCell - 2) + ":B" + Tostr(i + baseCell - 1)) 
