...
doc.setfont(zone1, "", -1, -1, 1)
 ...
 {
  for(i =0; i < k; i = i + 1)
   add(mass999,marc(source,999, i, $b"   "$p)) 	
  k = userselect(mass999)
  doc.out(marc(source,999, k, "   "$b"   "$p), Zone1)
 }
 else
  doc.out(marc(source,999, 0, "  "$b"   "$p), Zone1)
 ...
 for(var i =0; i< k; i =i +1)
 {
  ...
  {
   doc.out(source.value(686, i, $a), Zone2)
   break
  }
 }
 ...
 if(MARCEXIST(SOURCE,461, 0))
 {
  if((source.count(461, 0, 700) + source.count(461, 0, 701) + source.count(461, 0, 710) + source.count(461, 0, 711)) < 5)
  {
   if(marcexist(source, 461,0, 700, 0))
    doc.out(MARC(SOURCE, 461, 0, 700, 0, $a" "$b), zone6)
   else
    doc.out(MARC(SOURCE, 461, 0, 710, 0, $a". "$b), zone6)
  }
  doc.out("    ", zone4)
  var s
 ...
 else
 {
  if((source.count(700) + source.count(701) + source.count(710) + source.count(711)) < 5)
  {
   if(marcexist(source, 700, 0))
    doc.out(MARC(SOURCE, 700, 0, $a($b?" "$b)(!$b?", "$g)), zone6)
   else
    doc.out(MARC(SOURCE, 710, 0, $a". "($b?$b)(!$b?$g)), zone6)
  }
 }
 doc.out("    ", zone4)
 ...
 {
  if(source.exist(461))
   doc.out(MARC(SOURCE, 461, 0, 210, 0, $a[0]), zone4)
 }
 doc.out(MARC(SOURCE, 210, 0, ($d?", "$d[0])), zone4)
 
 doc.out("\n                             " + marc(source,999, 0, $u ), Zone4)
}